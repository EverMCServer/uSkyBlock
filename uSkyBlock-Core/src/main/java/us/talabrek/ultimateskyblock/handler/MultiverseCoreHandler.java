...
public enum MultiverseCoreHandler {;
    ...
    public static void importEndWorld(World skyWorld) {
        MultiverseCore core = getMultiverseCore();
        if (core != null) {
            if (!core.getMVWorldManager().isMVWorld(skyWorld)) {
                core.getMVWorldManager().addWorld(skyWorld.getName(), World.Environment.THE_END, "0", WorldType.NORMAL, false, "uSkyBlock", false);
            }
            MultiverseWorld mvWorld = core.getMVWorldManager().getMVWorld(skyWorld);
            mvWorld.setEnvironment(World.Environment.THE_END);
            mvWorld.setScaling(1);
            mvWorld.setGenerator("uSkyBlock");
            if (!Settings.extras_sendToSpawn) {
                mvWorld.setRespawnToWorld(Settings.general_worldName);
            ...
            if (!Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "mv import " + skyWorld.getName() + " END -g uSkyBlock")) {
                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "mv import " + skyWorld.getName() + " END uSkyBlock");
            }
        }
    }
