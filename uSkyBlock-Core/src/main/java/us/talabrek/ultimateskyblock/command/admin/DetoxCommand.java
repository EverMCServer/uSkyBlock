...
/**
 * /usb detox — removes the toxic water mark from all blocks in the WorldEdit selection.
 */
public class DetoxCommand extends RequirePlayerCommand {
    ...
    @Inject
    public DetoxCommand(@NotNull uSkyBlock plugin, @NotNull ToxicEvents toxicEvents) {
        super("detox", "usb.admin.detox", marktr("remove toxic mark from WE selection"));
        this.plugin = plugin;
    ...
    protected boolean doExecute(String alias, Player player, Map<String, Object> data, String... args) {
        ...
        player.sendMessage(tr("§aRemoving toxic mark from §e{0}§a blocks...", total));
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (long i = processed; i < end; i++) {
                    ...
                    toxicEvents.setToxic(b, false);
                }
                processed = end;
                if (processed >= total) {
                    player.sendMessage(tr("§aDone! §e{0}§a blocks cleared of toxic mark.", total));
                    this.cancel();
                }
            }
